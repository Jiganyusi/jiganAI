...
async function handleFileMessage(chatId, message, caption, env) {
  ...
  try {
    ...
    const systemPrompt = await buildBrainPrompt(room, caption || `Baca ${fileMeta.kind} ini.`);
...
async function processMentorMessage(chatId, room, text, env) { updateRoomConversation(room, "Mentor", text); updateMemoryFromConversation(room, text); const systemPrompt = await buildBrainPrompt(room, text); const providerResult = await callProvider(env, systemPrompt, text); const reply = buildBrainResponse(providerResult); updateRoomConversation(room, "Jiganyusi", reply); await saveRoom(env, chatId, room); return reply; }
