...
export async function handleTelegramUpdate(update, env) {
  ...
  if (!room) {
    await sendTelegramMessage(
      env.BOT_TOKEN,
      chatId,
      ["Mentor belum memilih Ruangan aktif.", "", "Silakan pilih salah satu tombol di bawah."].join("\n"),
      MAIN_MENU
    );
    return;
  }
  ...
  try {
    ...
    await sendTelegramMessage(
      ...
      chatId,
      [
        "Terjadi kendala saat memproses pesan, Mentor.",
        "",
...
async function showTopics(chatId, env) {
  ...
  if (!rooms.length) {
    await sendTelegramMessage(env.BOT_TOKEN, chatId, "Belum ada topik aktif, Mentor.\nGunakan Start untuk membuat ruangan baru.", MAIN_MENU);
    return;
  }
